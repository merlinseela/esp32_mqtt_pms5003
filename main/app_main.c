...
// SENSOR Read
pms5003_measurement_t sense_pms_read(pms5003_config_t pms0)
{
    pms5003_measurement_t reading;
    pms5003_make_measurement(&pms0, &reading); //reading.1
    pms5003_print_measurement(&reading);
    // we wait when we have published the data to the MQTT broker
...
{   
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        /* // sprintf(sensor_str, "%f", vals.pms25mat); 
        msg_id = esp_mqtt_client_publish(client, "/topic/qos1",sensor_str , 0, 1, 1);
...
static void mqtt5_app_start()
{
    ...
    */
    ...
    // Sensor setup
    pms5003_config_t pms0 = sense_pms_setup();

    // Sensor loop
    while (1)
    {
        //read sensor data/measurements
        pms5003_measurement_t vals = sense_pms_read(pms0);

        //mutate sensor data to fit MQTT payload
        char sensor_str[32];
        // publish sensor data to MQTT broker
        esp_mqtt5_client_set_user_property(&publish_property.user_property, user_property_arr, USE_PROPERTY_ARR_SIZE);
        esp_mqtt5_client_set_publish_property(client, &publish_property);
        snprintf(sensor_str, sizeof(sensor_str), "%d", vals.pm2_5_atm); //here we are sending pm2.5 data
        int msg_id = esp_mqtt_client_publish(client, "pms_m_t/atm_2.5", sensor_str, 0, 1, 1);
        msg_id=0;
        esp_mqtt5_client_delete_user_property(publish_property.user_property);
        publish_property.user_property = NULL;
